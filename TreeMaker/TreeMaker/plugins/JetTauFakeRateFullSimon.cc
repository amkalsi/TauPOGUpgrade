...
JetTauFakeRateFullSimon::JetTauFakeRateFullSimon(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
;
  ...
  h_Fill_DZ   = fs->make<TH1F>(("h_Fill_DZ"+prefix).c_str() ,"h_Fill_DZ",400,0,10);
   ...
   h_EMUveto_Response  = fs->make<TH1F>(("h_EMUveto_Response"+prefix).c_str() ,"h_EMUveto_Response",5000,0,50);


  h_EMUveto_GenJetPt  = fs->make<TH1F>(("h_EMUveto_GenJetPt"+prefix).c_str() ,"h_EMUveto_GenJetPt",100,0,700);
  h_EMUveto_GenJetEta  = fs->make<TH1F>(("h_EMUveto_GenJetEta"+prefix).c_str() ,"h_EMUveto_GenJetEta",100,-4,4);
  h_EMUveto_GenJetPhi  = fs->make<TH1F>(("h_EMUveto_GenJetPhi"+prefix).c_str() ,"h_EMUveto_GenJetPhi",100,-4,4);
  h_EMUveto_DR = fs->make<TH1F>(("h_EMUveto_DR"+prefix).c_str() ,"h_EMUveto_DR",500,0,8);
  h_EMUveto_JetPt = fs->make<TH1F>(("h_EMUveto_JetPt"+prefix).c_str() ,"h_EMUveto_JetPt",100,0,700); 
  h_EMUveto_JetEta = fs->make<TH1F>(("h_EMUveto_JetEta"+prefix).c_str() ,"h_EMUveto_JetEta",100,-4,4);
  h_EMUveto_JetPhi = fs->make<TH1F>(("h_EMUveto_JetPhi"+prefix).c_str() ,"h_EMUveto_JetPhi",100,-4,4);
  h_EMUveto_NVtx  = fs->make<TH1F>(("h_EMUveto_NVtx"+prefix).c_str() ,"h_EMUveto_NVtx",400,0,400);
  h_EMUveto_NGood  = fs->make<TH1F>(("h_EMUveto_NGood"+prefix).c_str() ,"h_EMUveto_NGood",400,0,400);
  h_EMUveto_NBest  = fs->make<TH1F>(("h_EMUveto_NBest"+prefix).c_str() ,"h_EMUveto_NBest",400,0,400);


  h_EMUveto_TauPt = fs->make<TH1F>(("h_EMUveto_TauPt"+prefix).c_str() ,"h_EMUveto_TauPt",100,0,700); 
  h_EMUveto_TauEta = fs->make<TH1F>(("h_EMUveto_TauEta"+prefix).c_str() ,"h_EMUveto_TauEta",100,-4,4);
  h_EMUveto_TauPhi = fs->make<TH1F>(("h_EMUveto_TauPhi"+prefix).c_str() ,"h_EMUveto_TauPhi",100,-4,4);


}


JetTauFakeRateFullSimon::~JetTauFakeRateFullSimon()
{
...
void JetTauFakeRateFullSimon::Fill(const edm::Event& iEvent,const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
...
for (reco::PFJetCollection::const_iterator jet2 = pfjets->begin(); jet2 != pfjets->end(); ++jet2) {
  ...
  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
    ...
    if(( abs(tauCandidate->eta())  > etacutmin) && (abs(tauCandidate->eta()) < etacut)) TauEtaCuts = true;
    ...
    if((corrJetP4.pt() > 20) && (fabs(corrJetP4.eta()) < etacut) && (fabs(corrJetP4.eta()) > etacutmin) ){
...
//
 if(((*_mv31)[tauCandidate] > 0.5 ) && ((*_enew)[tauCandidate] > 0.5 )){
      
   h_EMUveto_GenJetEta->Fill(unsmearedMomentum.eta());
   h_EMUveto_GenJetPhi->Fill(unsmearedMomentum.phi());
   h_EMUveto_GenJetPt->Fill(unsmearedMomentum.pt());
   h_EMUveto_DR->Fill(reco::deltaR(corrJetP4.eta(),corrJetP4.phi() , unsmearedMomentum.eta(),unsmearedMomentum.phi()));
   h_EMUveto_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
   h_EMUveto_JetPt->Fill(rawJetP4.pt());
   h_EMUveto_JetEta->Fill(rawJetP4.eta());
   h_EMUveto_JetPhi->Fill(rawJetP4.phi());
   h_EMUveto_NVtx->Fill(PUInfo_true);
   h_EMUveto_NGood->Fill(nGoodVtx);
   h_EMUveto_NBest->Fill(nBestVtx);

   h_EMUveto_TauPt->Fill(corrJetP4.pt());
   h_EMUveto_TauEta->Fill(corrJetP4.eta());
   h_EMUveto_TauPhi->Fill(corrJetP4.phi());
...
h_Loose3_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
	...
	h_Loose3_NVtx->Fill(PUInfo_true);
...
h_Medium3_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
	...
	h_Medium3_NVtx->Fill(PUInfo_true);
	...
	h_Tight3_NVtx->Fill(PUInfo_true);
	...
	h_VLooseChar_NVtx->Fill(PUInfo_true);
	...
	h_LooseChar_NVtx->Fill(PUInfo_true);
...
h_MediumChar_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());                                                          
	...
	h_MediumChar_NVtx->Fill(PUInfo_true);
...
h_TightChar_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());  
	      ...
	      h_TightChar_NVtx->Fill(PUInfo_true);
	      ...
	      h_LIsoPtSum_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
              ...
              h_LIsoPtSum_NVtx->Fill(PUInfo_true);
	    ...
	    //
	      ...
	      h_MIsoPtSum_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
              ...
              h_MIsoPtSum_NVtx->Fill(PUInfo_true);
	    ...
	    //
	      ...
	      h_TIsoPtSum_Response->Fill(corrJetP4.pt()/unsmearedMomentum.pt());
              ...
              h_TIsoPtSum_NVtx->Fill(PUInfo_true);
...
pair<bool, reco::Candidate::LorentzVector> JetTauFakeRateFullSimon::matchToGenJet(double jeteta , double jetphi , edm::Handle<reco::GenJetCollection> ak5jets) {                        
...
//  bool jetfound = false;
  ...
  for(reco::GenJetCollection::const_iterator genjet = ak5jets->begin();genjet != ak5jets->end();++genjet) {
    ...
    if(fabs(genjet->eta()) <= etacutmin) continue;
    if(fabs(genjet->eta()) >= etacut) continue;
...
bool JetTauFakeRateFullSimon::OverlapWithGenJet(double jeteta , double jetphi ,edm::Handle<reco::GenJetCollection> ak5jets ,int  &index,const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  double drtmp = _TauToGenMatchingDeltaR;
  int tmpindex =0 ;
  bool jetfound = false;
