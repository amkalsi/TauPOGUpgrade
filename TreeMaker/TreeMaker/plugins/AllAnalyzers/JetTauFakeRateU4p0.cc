...
JetTauFakeRateU4p0::JetTauFakeRateU4p0(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
JetTauFakeRateU4p0::~JetTauFakeRateU4p0()
{
...
void JetTauFakeRateU4p0::Fill(const edm::Event& iEvent,const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
  ...
  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
    ...
    if((corrJetP4.pt() > 20) && (fabs(corrJetP4.eta()) < 3.0) ){
...
*/
bool JetTauFakeRateU4p0::OverlapWithJet(double Taueta , double Tauphi ,edm::Handle<reco::PFJetCollection> jets ,int  &index,const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  double drtmp = 0.5;
  int tmpindex =0 ;
  bool jetfound = false;
