...
MuTauFakeRateU4p0::MuTauFakeRateU4p0(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
MuTauFakeRateU4p0::~MuTauFakeRateU4p0()
{
...
void MuTauFakeRateU4p0::Fill(const edm::Event& iEvent) {
  using namespace edm;
  using namespace std;
	...
	if(abs(genparticles->mother(0)->pdgId()) == _TauMotherId) {
	  ...
	  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
	    ...
	    if(reco::deltaR(genparticles->eta(), genparticles->phi(), tauCandidate->eta(), tauCandidate->phi()) < _TauToGenMatchingDeltaR) {
	      ...
	      if(DecayModeFinding  && TauEtaCuts && RecoptCut && DecayModeFinding && IsoDisc ) {
		if (MuonDisc) { 
...
bool MuTauFakeRateU4p0::IsZMuMu(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZMuMu = false;
...
bool MuTauFakeRateU4p0::OverlapWithGenMu(double Taueta , double Tauphi ,edm::Handle<reco::GenParticleCollection> genParticles ,int  &index ) {
