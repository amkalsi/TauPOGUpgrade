...
MuTauFakeRateNew::MuTauFakeRateNew(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
MuTauFakeRateNew::~MuTauFakeRateNew()
{
...
void MuTauFakeRateNew::Fill(const edm::Event& iEvent) {
  using namespace edm;
  using namespace std;
...
bool MuTauFakeRateNew::OverlapWithGenTauForOther(double Taueta , double Tauphi , edm::Handle<reco::PFTauCollection> taujetcol, edm::Handle<reco::PFTauDiscriminator>  disc ,int &index ) {
...
bool MuTauFakeRateNew::IsZMuMu(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZMuMu = false;
...
bool MuTauFakeRateNew::OverlapWithGenMu(double Taueta , double Tauphi ,edm::Handle<reco::GenParticleCollection> genParticles ,int  &index ) {
  double drtmp = _TauToGenMatchingDeltaR;
  int tmpindex =0 ;
  bool genTaufound = false;
