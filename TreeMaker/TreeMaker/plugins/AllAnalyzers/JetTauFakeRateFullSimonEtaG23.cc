...
JetTauFakeRateFullSimonEtaG23::JetTauFakeRateFullSimonEtaG23(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
JetTauFakeRateFullSimonEtaG23::~JetTauFakeRateFullSimonEtaG23()
{
...
void JetTauFakeRateFullSimonEtaG23::Fill(const edm::Event& iEvent,const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
...
for (reco::PFJetCollection::const_iterator jet2 = pfjets->begin(); jet2 != pfjets->end(); ++jet2) {
  ...
  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
    ...
    if( (abs(tauCandidate->eta()) >= 2.3) && (abs(tauCandidate->eta()) < 4.0)) TauEtaCuts = true;
    ...
    if((corrJetP4.pt() > 20) && (fabs(corrJetP4.eta()) < 4.0) && (fabs(corrJetP4.eta()) > 2.3) ){
...
pair<bool, reco::Candidate::LorentzVector> JetTauFakeRateFullSimonEtaG23::matchToGenJet(double jeteta , double jetphi , edm::Handle<reco::GenJetCollection> ak5jets) {                        
...
//  bool jetfound = false;
  ...
  for(reco::GenJetCollection::const_iterator genjet = ak5jets->begin();genjet != ak5jets->end();++genjet) {
    ...
    if(fabs(genjet->eta()) < 2.3) continue;
...
bool JetTauFakeRateFullSimonEtaG23::OverlapWithGenJet(double jeteta , double jetphi ,edm::Handle<reco::GenJetCollection> ak5jets ,int  &index,const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  double drtmp = _TauToGenMatchingDeltaR;
  int tmpindex =0 ;
  bool jetfound = false;
