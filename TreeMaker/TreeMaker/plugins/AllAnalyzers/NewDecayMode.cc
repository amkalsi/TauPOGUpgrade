...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 int igentau = 0;
 int index  = -1; 
 if(OverlapWithGenTau( gentaujet->eta() , gentaujet->phi() , _hpsTaucoll, index ) && index != -1 ){                      
	 ...
	 if( (tauCandidate->pt() > 20) && (fabs(tauCandidate->eta()) < 2.3) ) {
	   ...
	   if((*_Newdmf)[tauCandidate] > 0.5) { 

	     // input variables to mVA veto ...................................................................................	     
	     if((tauCandidate->leadPFChargedHadrCand().isNonnull()) && (tauCandidate->leadPFChargedHadrCand()->p() > 0)) {     
	     ...
	     for ( std::vector<PFCandidatePtr>::const_iterator signalTrack =  tauCandidate->signalPFGammaCands().begin() ;signalTrack != tauCandidate->signalPFGammaCands().end(); ++signalTrack ) {
	       sumPt2 += (*signalTrack)->pt();
             ...
             if(tauCandidate->pt() > 0 ){ FillGammaFraction_DMF->Fill(sumPt2/tauCandidate->pt()); }
...
igentau++;
   ...
   //////////////////////////////////////////////////Other Discriminator Efficiency //////////////////////////////////////////////
 ...
 if(OverlapWithGenTau( gentaujet->eta() , gentaujet->phi() , _hpsTaucoll, index ) && index != -1 ){                      
     ...
     for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
       reco::PFTauRef tauCandidate(_hpsTaucoll, iTau);
         if(igentau == index) {     
       if( (tauCandidate->pt() > 20) && (fabs(tauCandidate->eta()) < 2.3) && ((*_Newdmf)[tauCandidate] > 0.5) ) {
	
	 if(reco::deltaR(gentaujet->eta(), gentaujet->phi(), tauCandidate->eta(), tauCandidate->phi()) < _TauToGenMatchingDeltaR) {
	   
	   // Denominator for other discriminators

	   if(gentaujet->pt() != 0) {
	     TauPtResponse_DenDMF->Fill(tauCandidate->pt()/gentaujet->pt());
	   }

	   RecoTauDecayMode_DenDMF->Fill(tauCandidate->decayMode());
	   GenTauDecayMode_DenDMF->Fill(tauGenJetDecayModeInt);
	   reco_dm_vs_gen_DenDMF->Fill(tauGenJetDecayModeInt,tauCandidate->decayMode());
	   GenTauPtWithDMF_DenDMF->Fill(gentaujet->pt());
	   GenTauEtaWithDMF_DenDMF->Fill(gentaujet->eta());
	   GenTauPhiWithDMF_DenDMF->Fill(gentaujet->phi());
...
bool NewDecayMode::OverlapWithGenTau(double Taueta , double Tauphi ,edm::Handle<reco::PFTauCollection> _hpsTaucoll ,int  &index     ) {
