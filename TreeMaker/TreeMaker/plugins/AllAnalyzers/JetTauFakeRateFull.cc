...
JetTauFakeRateFull::JetTauFakeRateFull(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
JetTauFakeRateFull::~JetTauFakeRateFull()
{
...
void JetTauFakeRateFull::Fill(const edm::Event& iEvent,const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
...
for (reco::PFJetCollection::const_iterator jet2 = pfjets->begin(); jet2 != pfjets->end(); ++jet2) {
  ...
  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
    ...
    if((*_hpsTau)[tauCandidate] < 1.0) IsoDiscVLC = true; 
    
    if((*_hpsTau1)[tauCandidate] < 1.0) IsoDiscLC = true; 

    if((*_hpsTau2)[tauCandidate] < 1.0) IsoDiscMC = true; 

    if((*_hpsTau3)[tauCandidate] < 1.0) IsoDiscTC = true; 
...
pair<bool, reco::Candidate::LorentzVector> JetTauFakeRateFull::matchToGenJet(double jeteta , double jetphi , edm::Handle<reco::GenJetCollection> ak5jets) {                        
...
bool JetTauFakeRateFull::OverlapWithGenJet(double jeteta , double jetphi ,edm::Handle<reco::GenJetCollection> ak5jets ,int  &index,const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  double drtmp = _TauToGenMatchingDeltaR;
  int tmpindex =0 ;
  bool jetfound = false;
