...
JetTauFakeRateG2p3::JetTauFakeRateG2p3(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
JetTauFakeRateG2p3::~JetTauFakeRateG2p3()
{
...
void JetTauFakeRateG2p3::Fill(const edm::Event& iEvent,const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
  ...
  for ( unsigned iTau = 0; iTau < _hpsTaucoll->size(); ++iTau ) {
    ...
    if((corrJetP4.pt() > 20) && (fabs(corrJetP4.eta()) > 2.3) && (fabs(corrJetP4.eta()) < 3.0) ){
...
*/
bool JetTauFakeRateG2p3::OverlapWithJet(double Taueta , double Tauphi ,edm::Handle<reco::PFJetCollection> jets ,int  &index,const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  double drtmp = 0.5;
  int tmpindex =0 ;
  bool jetfound = false;
