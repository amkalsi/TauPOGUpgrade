...
ETauFakeRateNew::ETauFakeRateNew(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
ETauFakeRateNew::~ETauFakeRateNew()
{
...
void ETauFakeRateNew::Fill(const edm::Event& iEvent) {
  using namespace edm;
  using namespace std;
...
if((*_enew)[tauCandidate] > 0.5) {
  ...
  if((tauCandidate->leadPFChargedHadrCand().isNonnull()) && (tauCandidate->leadPFChargedHadrCand()->p() > 0)) {       
     ...
     FillHOverP_after->Fill(tauCandidate->leadPFChargedHadrCand()->hcalEnergy()/tauCandidate->leadPFChargedHadrCand()->p());
     FillEOverP_after->Fill(tauCandidate->leadPFChargedHadrCand()->ecalEnergy()/tauCandidate->leadPFChargedHadrCand()->p());
     
   }
   
   hFillTauHcal3x3OverPLead_after->Fill(tauCandidate->hcal3x3OverPLead());
   ...
   if(tauCandidate->pt() > 0 ){ FillGammaFraction_after->Fill(sumPt1/tauCandidate->pt()); }
...
bool ETauFakeRateNew::OverlapWithGenTauForOther(double Taueta , double Tauphi , edm::Handle<reco::PFTauCollection> taujetcol, edm::Handle<reco::PFTauDiscriminator>  disc ,int &index ) {
...
bool ETauFakeRateNew::IsZEE(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZEE = false;
...
bool ETauFakeRateNew::OverlapWithGenMu(double Taueta , double Tauphi ,edm::Handle<reco::GenParticleCollection> genParticles ,int  &index ) {
  double drtmp = _TauToGenMatchingDeltaR;
  int tmpindex =0 ;
  bool genTaufound = false;
