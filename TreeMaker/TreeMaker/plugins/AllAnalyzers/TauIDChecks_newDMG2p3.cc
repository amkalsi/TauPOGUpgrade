...
TauIDChecks_newDMG2p3::TauIDChecks_newDMG2p3(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
 {
...
void TauIDChecks_newDMG2p3::Fill(const edm::Event& iEvent) {
   using namespace edm;
   using namespace std;
...
bool TauIDChecks_newDMG2p3::OverlapWithGenTau(double Taueta , double Tauphi ,edm::Handle<std::vector< reco::GenJet > > genTauJets ,int  &index     ) {
...
bool TauIDChecks_newDMG2p3::IsZTauTau(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZTauTau = false;
