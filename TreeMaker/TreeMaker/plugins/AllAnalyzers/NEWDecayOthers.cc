...
NEWDecayOthers::NEWDecayOthers(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
 ...
 h_RunNumber = fs->make<TH1F>(("h_RunNumber"+prefix).c_str() ,"h_RunNumber",1000000,0,1000000);                                                                  
  h_EventNumber = fs->make<TH1F>(("h_EventNumber"+prefix).c_str() ,"h_EventNumber",1000000,0,1000000);
  h_LumiNumber = fs->make<TH1F>(("h_LumiNumber"+prefix).c_str() ,"h_LumiNumber",1000000,0,1000000);
...
////////////////////////// individual decay modes
   ...
   for(reco::GenJetCollection::const_iterator gentaujet = genTauJets->begin(); gentaujet !=  genTauJets->end(); ++gentaujet) { 
     ...
     if(gentaujet->pt() < 100 ) continue;
     if(gentaujet->pt() > 120 ) continue;
     ...
     if(tauGenJetDecayMode=="oneProng0Pi0") {
       ...
       cout<<"tauPt All\t"<<gentaujet->pt()<<endl;
       cout<<"tauEta All \t"<<gentaujet->eta()<<endl;
       cout<<"tauPhi All \t"<<gentaujet->phi()<<endl;

       cout<<"run number All \t"<<iEvent.id().run()<<endl;
       cout<<"event number All \t"<<iEvent.id().event()<<endl;
       cout<<"lumi number All \t"<<iEvent.id().luminosityBlock()<<endl;
   ...
   cout<<"tauPt \t"<<gentaujet->pt()<<endl;
   cout<<"tauEta \t"<<gentaujet->eta()<<endl;
   cout<<"tauPhi \t"<<gentaujet->phi()<<endl;

   cout<<"run number \t"<<iEvent.id().run()<<endl;
   cout<<"event number\t"<<iEvent.id().event()<<endl;
   cout<<"lumi number \t"<<iEvent.id().luminosityBlock()<<endl;
	 h_RunNumber->Fill(iEvent.id().run()); 
   h_EventNumber->Fill(iEvent.id().event());
   h_LumiNumber->Fill(iEvent.id().luminosityBlock());
	 if(gentaujet->pt() != 0) {
...
bool NEWDecayOthers::OverlapWithGenTau(double Taueta , double Tauphi , edm::Handle<reco::PFTauCollection> taujetcol, edm::Handle<reco::PFTauDiscriminator>  disc, int &index ) {
   ...
   for ( unsigned iTau = 0; iTau < taujetcol->size(); ++iTau ) {
     ...
     if(tauCandidate->pt() < 100) continue;
     if(tauCandidate->pt() > 120) continue;
     cout<<"passes tau pt cut"<<endl;
     if(fabs(tauCandidate->eta()) >= 2.3) continue;
     cout<<"passes tau eta cut"<<endl;
     if((*disc)[tauCandidate] > 0.5) {
     cout<<"passed DMF New"<<endl;
       double dr =reco::deltaR(Taueta, Tauphi,tauCandidate->eta(),tauCandidate->phi());
       if(dr < drtmp){
   cout<<"passing DR cut"<<"\t DR value \t"<<dr<<endl;
	 drtmp = dr;
	 index = iTau;
	 genTaufound= true;
       }
