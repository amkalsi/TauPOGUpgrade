...
TauIDChecks::TauIDChecks(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
 {
...
void TauIDChecks::Fill(const edm::Event& iEvent) {
   using namespace edm;
   using namespace std;
...
bool TauIDChecks::OverlapWithGenTau(double Taueta , double Tauphi ,edm::Handle<std::vector< reco::GenJet > > genTauJets ,int  &index     ) {
...
bool TauIDChecks::IsZTauTau(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZTauTau = false;
