...
TauIDUpto2to3New::TauIDUpto2to3New(std::string prefix,edm::Service<TFileService> fs,const edm::ParameterSet& iConfig)
{
...
TauIDUpto2to3New::~TauIDUpto2to3New()
{
...
void TauIDUpto2to3New::Fill(const edm::Event& iEvent) {
   using namespace edm;
   using namespace std;
...
bool TauIDUpto2to3New::OverlapWithGenTau(double Taueta , double Tauphi , edm::Handle<reco::PFTauCollection> taujetcol, edm::Handle<reco::PFTauDiscriminator>  disc, int &index ) {
...
bool TauIDUpto2to3New::OverlapWithGenTauForOther(double Taueta , double Tauphi , edm::Handle<reco::PFTauCollection> taujetcol, edm::Handle<reco::PFTauDiscriminator>  disc, edm::Handle<reco::PFTauDiscriminator>  disc1 ,int &index ) {
   double drtmp = _TauToGenMatchingDeltaR;
   bool genTaufound = false;
   index = -1;
   for ( unsigned iTau = 0; iTau < taujetcol->size(); ++iTau ) {
...
bool TauIDUpto2to3New::IsZTauTau(edm::Handle<reco::GenParticleCollection> genParticles){
   bool isZTauTau = false;
